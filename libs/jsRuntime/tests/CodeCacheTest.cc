...
namespace v8App
{
    namespace JSRuntime
    {
        ...
        V8MaybeLocalModule UnresovleCdallback(
        ...
        int32_t ExecuteScript(V8Isolate *isolate, V8ScriptSource *source, bool useCache)
        {
            ...
            module->InstantiateModule(context, UnresovleCdallback);
            if (tryCatch.HasCaught())
            {
                std::string stack = JSUtilities::GetStackTrace(isolate, tryCatch);
                std::cout << stack << std::endl;
        ...
        V8ScriptCachedData *GenerateCodeCache(V8Isolate *inIsolate, V8LocalContext inContet, V8ScriptSource *inSource)
        {
            ...
            module->InstantiateModule(inContet, UnresovleCdallback);
        ...
        TEST_F(CodeCacheTest, LoadScriptFileSetCacheData)
        {
            ...
            ASSERT_TRUE(CodeCacheTestInternal::WriteTestFile(testPath, "function f(){return 2;}(function() { globalThis.Result=f(); })()"));
            ...
            EXPECT_EQ(2, ExecuteScript(m_Isolate, source.get(), false));

            source = codeCache.LoadScriptFile(testPath, m_Isolate);
            V8ScriptCachedData *cache = GenerateCodeCache(m_Isolate, context, source.get());
            ...
            EXPECT_EQ(2, ExecuteScript(m_Isolate, source.get(), true));
            ...
            EXPECT_EQ(2, ExecuteScript(m_Isolate, source.get(), true));
            {
                ...
                EXPECT_EQ(2, ExecuteScript(m_Isolate, source2.get(), true));
            }

            // js file changed
            ASSERT_TRUE(CodeCacheTestInternal::WriteTestFile(testPath, "function f(){return 4;}(function() { globalThis.Result=f(); })()"));
            source = codeCache.LoadScriptFile(testPath, m_Isolate);
            ASSERT_NE(nullptr, source);
            EXPECT_EQ(nullptr, source->GetCachedData());

            EXPECT_EQ(4, ExecuteScript(m_Isolate, source.get(), false));
        }
        ...
        TEST_F(CodeCacheTest, CreateCacheInfo)
        {
            ...
            ASSERT_TRUE(CodeCacheTestInternal::WriteTestFile(testPath, sourceStr));
