...
TEST_F(RunfilesTest, ManifestBasedRunfilesRlocationWildAndEnvVars) {
  unique_ptr<MockFile> mf(
      MockFile::Create("foo" LINE_AS_STRING() ".runfiles_manifest", {"a/bcd/e f/g"}));
  EXPECT_TRUE(mf != nullptr);

  string error;
  unique_ptr<Runfiles> r(Runfiles::Create("ignore-argv0", mf->Path(), "", &error));
  ASSERT_NE(r, nullptr);
  EXPECT_TRUE(error.empty());

  EXPECT_EQ(r->Rlocation("?/bcd/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/?cd/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/??d/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/\?\?\?/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/bcd/?"), "f/g");
  EXPECT_EQ(r->Rlocation("a/*/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/?*/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/*?/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/**/e"), "f/g");
  EXPECT_EQ(r->Rlocation("*/e"), "f/g");
  EXPECT_EQ(r->Rlocation("*/bcd/e"), "f/g");
  EXPECT_EQ(r->Rlocation("a/bcd/*"), "f/g");
  EXPECT_EQ(r->Rlocation("\?\?/foo"), "");
  EXPECT_EQ(r->Rlocation("*/*/foo"), "");
  EXPECT_EQ(r->Rlocation("b/*/c"), "");

  EXPECT_EQ(r->Rlocation("a/?ef"), "");
}
