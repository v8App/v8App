  ...
  namespace tools
  {
    namespace cpp
    {
      namespace runfiles
      {
        ...
        string Runfiles::RlocationWild(const string &path) const
        {
          ...
          {
            string::value_type c = path[i];
            if (c == '*')
            {
              //we want to collapse the ? or multiple * into a single * if they are next to each other
              if (token.empty() == false && inMulti == false && inSingle == false)
              {
            ...
            if (c == '?')
            {
              if (token.empty() == false && inSingle == false && inMulti == false)
              {
                tokens.push_back(token);
                token = string();
              }
              //if we are in a multi collapse into it
              if (inMulti)
              {
                continue;
              }
            ...
            //no wild card character at this point
            //if inSingle then we need to end it and push it
            if (inSingle || inMulti)
            {
              tokens.push_back(token);
              token = string();
          ...
          //ok tokenzied the string now to start comparing the string
          for (auto &it : runfiles_map_)
          {
            string key = it.first;
            size_t keyIdx = 0;
            ...
            for (; i < tokens.size(); i++)
            {
              ...
              if (token[0] == '*')
              {
                //if this is the last token then we can skip the rest
                if (i == tokens.size() - 1)
                {
                  keyIdx = keySize;
                  continue;
                }
                //we adavance the counter
                token = tokens[++i];
                size_t t = 0;
                do {
                  //find a character that matche the token's first one
                  while (keyIdx < keySize && token[0] != key[keyIdx])
                  {
                    keyIdx++;
                  }
                  //loop the token until it's fully matched or break if we don't find a match
                  for(t = 0; t < token.size() && keyIdx < keySize; t++, keyIdx++) {
                    if(key[keyIdx] != token[t]) {
                      t = 0;
                      break;
                    }
                  }
                  //we eiather end up hitting the end of the token as matched to the end of the key
                } while(keyIdx < keySize && t < token.size());
                //if we were unable to match the full token
                if(t != token.size()) {
                  break;
                }
                continue;
            ...
            if (keyIdx == keySize && i == tokens.size())
            {
              return it.second;
            }
          }
        ...
        string Runfiles::Rlocation(const string &path) const
        {
          ...
          //see if the path has wildcards
          if (contains(path, "?") || contains(path, "*"))
          {
            return RlocationWild(path);
          }
